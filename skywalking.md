```xml
<?xml version="1.0" encoding="UTF-8"?>

<!-- Reference Manual http://logback.qos.ch/manual/index.html -->

<configuration>

    <springProperty scope="context" name="APP_NAME" source="spring.application.name"/>
    <property name="APP_NAME" value="${APP_NAME}"/>
    <property name="LOG_PATH" value="${LOG_PATH:-${user.dir:-/tmp/${APP_NAME}}/logs}"/>

    <property name="LOG_PATTERN" value="%d{yyyy-MM-dd HH:mm:ss.SSS} %-5level [%tid] [%thread] %logger{36}:%L - %msg%n"/>

    <appender name="CONSOLE" class="ch.qos.logback.core.ConsoleAppender">
        <encoder class="ch.qos.logback.core.encoder.LayoutWrappingEncoder">
            <layout class="org.apache.skywalking.apm.toolkit.log.logback.v1.x.TraceIdPatternLogbackLayout">
                <pattern>${LOG_PATTERN}</pattern>
            </layout>
        </encoder>
    </appender>

    <appender name="FILE" class="ch.qos.logback.core.rolling.RollingFileAppender">
        <file>${LOG_PATH}/${APP_NAME}.log</file>

        <filter class="ch.qos.logback.classic.filter.ThresholdFilter">
            <level>DEBUG</level>
        </filter>

        <!-- 可让每天产生一个日志文件，最多 10 个，自动回滚 -->
        <rollingPolicy class="ch.qos.logback.core.rolling.TimeBasedRollingPolicy">
            <fileNamePattern>${LOG_PATH}/old/${APP_NAME}.log-%d{yyyy-MM-dd}</fileNamePattern>
            <maxHistory>10</maxHistory>
        </rollingPolicy>
        <encoder class="ch.qos.logback.core.encoder.LayoutWrappingEncoder">
            <layout class="org.apache.skywalking.apm.toolkit.log.logback.v1.x.TraceIdPatternLogbackLayout">
                <pattern>${LOG_PATTERN}</pattern>
            </layout>
        </encoder>
    </appender>

    <!-- 输出到控制台和文件，可定义更多的 Appender -->
    <root level="DEBUG">
        <appender-ref ref="CONSOLE"/>
        <appender-ref ref="FILE"/>
    </root>

    <!--&lt;!&ndash; 日志输出格式 &ndash;&gt;-->
    <property name="BIZ_LOG_PATTERN" value="%d{yyyy-MM-dd HH:mm:ss.SSS} %-5level [%thread] %logger{36}:%L - %msg%n"/>

    <appender name="BIZ_FILE" class="ch.qos.logback.core.rolling.RollingFileAppender">

        <file>${LOG_PATH}/${APP_NAME}.json</file>

        <filter class="ch.qos.logback.classic.filter.ThresholdFilter">
            <level>DEBUG</level>
        </filter>

        <!-- 可让每天产生一个日志文件，最多 10 个，自动回滚 -->
        <rollingPolicy class="ch.qos.logback.core.rolling.TimeBasedRollingPolicy">
            <fileNamePattern>${LOG_PATH}/old/${APP_NAME}.json-%d{yyyy-MM-dd}</fileNamePattern>
            <maxHistory>10</maxHistory>
        </rollingPolicy>

        <encoder>
            <pattern>${BIZ_LOG_PATTERN}</pattern>
            <charset>utf8</charset>
        </encoder>

    </appender>

    <logger name="com.kingxunlian.log.BusinessLogger" level="INFO" additivity="false">
        <appender-ref ref="BIZ_FILE"/>
    </logger>

    <!-- 下面配置一些第三方包的日志过滤级别，用于避免刷屏 -->
    <logger name="org.hibernate" level="INFO"/>
    <logger name="org.springframework" level="INFO"/>
    <logger name="org.jboss" level="INFO"/>
    <logger name="org.apache" level="INFO"/>
    <logger name="springfox.documentation" level="INFO"/>
    <logger name="javax.management" level="INFO"/>

    <logger name="com.netflix.loadbalancer.DynamicServerListLoadBalancer" level="INFO" />
    <logger name="com.netflix.loadbalancer.BaseLoadBalancer" level="INFO" />
    <logger name="com.kingxunlian.heaven.client.DefaultConfigFetcher" level="INFO"/>
    <logger name="com.kingxunlian.heaven.client.DefaultConfigUpdater" level="INFO"/>

    <logger name="com.netflix.discovery.DiscoveryClient" level="INFO"/>
    <logger name="com.netflix.discovery.shared.NamedConnectionPool" level="INFO"/>
    <logger name="com.netflix.discovery.shared.MonitoredConnectionManager" level="INFO"/>
    <logger name="com.netflix.discovery.shared.transport.jersey.AbstractJerseyEurekaHttpClient" level="INFO"/>

    <logger name="com.netflix.loadbalancer.ZoneAffinityServerListFilter" level="INFO"/>

</configuration>

```

